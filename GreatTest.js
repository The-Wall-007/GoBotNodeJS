...
async function processBookingStep(userInput, userSession) {
  ...
  async function extractBookingInfo(input) {
    ...
    if (locationText && !extracted.pickupLocation) {
      ...
      if (locationMatches.length === 1) {
        ...
        extracted.pickupLocation = null; //No location found
      }
    } else {
    ...
    if (parsedDate) {
      ...
      extracted.pickupTime = format(parsedDate, "HH:mm"); // Extract time if present
    } else {
  ...
  switch (currentStep) {
    ...
    // case "pickupDate":
    // case "pickupTime": {
    //   // These three steps now use the same logic
    ...
    //     if (locationMatches.length === 1) {
    //       bookingDetails.pickupLocation = locationMatches[0];
    //       botResponse = `Pickup location set to ${locationMatches[0].label}. What is the pickup date?`;
    //       userSession.currentStep = "pickupDate";
    //     } else if (locationMatches.length > 1) {
    //       botResponse = `I found multiple matches for your pickup location: ${locationMatches
    //         .map((loc) => loc.label)
    ...
    case "pickupTime": {
      ...
      console.log(
        JSON.stringify(bookingDetails.pickupLocation) +
          "\n" +
